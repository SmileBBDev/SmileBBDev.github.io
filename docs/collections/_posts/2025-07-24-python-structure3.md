...
- Indexing : 인덱싱으로 특정 위치 요소 접근
  ```python
  abc = [1, 2, "3"]
  print(abc[0])  # 1
  ```
- Slicing : 범위 지정하여 일정 부분 추출
  ```python
  print(abc[1:-1])  # [2]
  print(abc[::2])   # [1, "3"]
  ```
- Concatenation (+) : 자료구조 단위 결합
  ```python
...
- Membership Test (in) : 포함 여부 확인 (True/False 반환)
  ```python
...
### 분류기준 2 - 수정이 가능한가?
#### 수정 가능한 자료형 (Mutable) - list, dict, set
#### 수정 불가능한 자료형 (Immutable) - str, tuple
...
파이썬에서는 리스트 안에 리스트를 넣어 2차원 구조를 자주 사용
- append() : 요소 추가
- extend() : 여러 요소 확장
